%%% @doc Periodic monitoring for workbench_metrics.
%%%
...
%%% scheduler utilization</a> and
%%% <a href="http://www.erlang.org/doc/reference_manual/distributed.html">
%%% Erlang distribution</a> sockets.
%%%
%%% We explicitly handle e.g. scheduler utilization here because doing it
%%% with the exometer 'function' type (with caching) resulted in the scheduler
%%% stats "flatlining" at some point after startup, sometimes a few hours.
-module(workbench_metrics_periodic).
...
%% The time percentage the scheduler has been running processes, NIFs,
%% BIFs, garbage collection, etc. versus time spent idling or trying to
%% schedule processes.
export_scheduler_usage({SchedulerId, Usage}) ->
   workbench_metrics:gauge([recon, scheduler, usage, SchedulerId], Usage).

%%% Functions supporting distribution port monitoring.

%% Convert a node name atom (e.g. workbench1@127.0.0.1) to a form suitable
%% for use in a exometer metric name (e.g. workbench1_127_0_0_1).
...
export_port_datum({Node, {Key, Value}}) ->
   workbench_metrics:gauge([dist, Key, Node], Value).
